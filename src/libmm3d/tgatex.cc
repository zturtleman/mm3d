...
Texture::ErrorE LoadUncompressedTGA(Texture *, const char *, FILE *);	// Load an Uncompressed file
Texture::ErrorE LoadCompressedTGA(Texture *, const char *, FILE *);		// Load a Compressed file
...
Texture::ErrorE TgaTextureFilter::readFile(Texture * texture, const char * filename)
{
   ...
   {
      err = LoadUncompressedTGA(texture, filename, fp);
   }
   else if(memcmp(cTGAcompare, &tgaheader.Header, sizeof(tgaheader.Header)) == 0)
   {
      err = LoadCompressedTGA(texture, filename, fp);
   }
   else
   {
...
Texture::ErrorE LoadUncompressedTGA(Texture * texture, const char * filename, FILE * fp)
{
   log_debug( "loading uncompressed TGA\n" );

   TgaTextureFilter::read( tga.width, fp );
   TgaTextureFilter::read( tga.height, fp );
   TgaTextureFilter::read( tga.bpp, fp );
   TgaTextureFilter::read( tga.reserved, fp );
...
Texture::ErrorE LoadCompressedTGA(Texture * texture, const char * filename, FILE * fp)
{ 
...
void TgaTextureFilter::read( uint8_t & val, FILE * fp )
{
   fread( &val, sizeof(val), 1, fp );
}

void TgaTextureFilter::read( uint16_t & val, FILE * fp )
{
   fread( &val, sizeof(val), 1, fp );
   val = ltoh_16( val );
}

void TgaTextureFilter::readBytes( void * buf, size_t len, FILE * fp )
{
   fread( buf, len, 1, fp );
}
